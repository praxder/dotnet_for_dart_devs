    ...
    : BackgroundService
{
    ...
    private async Task SendDailyDigestsAsync(CancellationToken ct)
    {
        ...
        var recipients = await subscribers.GetActiveDigestRecipientsAsync(ct);

        logger.LogInformation("Sending digest to {Count} subscribers", recipients.Count);
...
[Queue("default")]
public class ReportJobHandler(IReportRepository reports, IStorageService storage)
{
    public async Task GenerateDailyReportAsync(PerformContext? context)
    {
        var yesterday = DateTime.UtcNow.Date.AddDays(-1);

        var report = await reports.GetDailyOrderSummaryAsync(yesterday);
...
builder.Services.AddQuartz(q =>
{
    ...
    q.AddTrigger(opts => opts
        ...
        .WithCronSchedule("0 0 6 * * ?",  // 6 AM every day
            x => x.InTimeZone(TimeZoneInfo.FindSystemTimeZoneById("America/New_York")))
        .StartNow());
});
...
// Job implementation
public class DailyReportJob(IReportService reports, ILogger<DailyReportJob> logger) : IJob
{

...
// Producer — persist the order and outgoing message atomically.
    ...
    IOutboxWriter outbox)
{
    public async Task<Order> PlaceOrderAsync(PlaceOrderRequest request, CancellationToken ct)
    {
        ...
        await orders.InsertAsync(connection, transaction, order, ct);
        await outbox.AppendAsync(connection, transaction, new OrderPlaced(
...
// An outbox dispatcher publishes committed messages through IBus and marks them sent.
// Consumer — separate process or background service
public class OrderPlacedConsumer(IEmailService email, ICustomerRepository customers)
    : IConsumer<OrderPlaced>
{
    public async Task Consume(ConsumeContext<OrderPlaced> context)
    {
        var customer = await customers.GetByIdAsync(context.Message.CustomerId);
...
The outbox closes the crash window between committing the order and publishing its event. Dispatch is still at-least-once: consumers must be idempotent, and the dispatcher must retry without deleting an event before the broker confirms publication.
...
public class InventoryCheckConsumer(IProductRepository products)
    : IConsumer<CheckInventoryRequest>
{
    public async Task Consume(ConsumeContext<CheckInventoryRequest> context)
    {
        var product = await products.GetByIdAsync(context.Message.ProductId);

---
title: "Capstone Project: Integration Plan and Architecture"
day: 129
week: 26
module: 10
...
This capstone consolidates work from the backend modules into one releasable system. Day 129 is an architecture and integration checkpoint, not a claim that a production application can be designed from scratch in 45 minutes. Reuse the API, Dapper repositories, authentication, tests, container, and CI work you built earlier.
...
│
├── TaskBoard.Infrastructure/      ← Dapper, Identity store, external services
│   ├── Persistence/
...
<ExerciseBlock>
1. **Architecture review**: Before integration, document your architecture decisions: (a) Is CQRS + MediatR earning its complexity for this application? (b) How will Dapper map `Label[]` and persist board aggregates without hidden N+1 queries? (c) How are concurrent task moves detected and resolved? (d) How does the Flutter app handle the SignalR connection when it enters the background?
...
3. **Data layer**: Write reviewed PostgreSQL migrations and Dapper mappings for the task board. Cover `TaskPosition`, labels in normalized tables or `jsonb`, workspace membership with roles, indexes, foreign keys, and an optimistic-concurrency version. Seed a test workspace with three boards and verify repository behavior with an integration test.
</ExerciseBlock>

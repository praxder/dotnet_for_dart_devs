...
// 4. Build the Dapper data source after Key Vault configuration is loaded.
// The connection string secret is retrieved at runtime, not committed to the app.
var connectionString = builder.Configuration.GetConnectionString("Default")
    ?? throw new InvalidOperationException("Missing database connection string");
builder.Services.AddSingleton(NpgsqlDataSource.Create(connectionString));
```

Some Azure database products also support passwordless Entra ID connections. Follow the current provider documentation and use a driver-supported token refresh mechanism. Do not invent an EF Core option or assign a one-time access token that will expire inside a long-lived connection pool.

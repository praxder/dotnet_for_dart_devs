...
// Persist the aggregate and its outgoing events atomically with Dapper.
    ...
    IOutboxWriter outbox)
{
    public async Task HandleAsync(Order order, CancellationToken cancellationToken)
    {
        ...
        await orders.UpdateAsync(connection, transaction, order, cancellationToken);
        await outbox.AppendAsync(connection, transaction, order.DomainEvents, cancellationToken);
...
An outbox worker publishes committed events and records delivery progress. Publishing directly after a database commit can lose an event if the process stops between the commit and publish; publishing before commit can announce a change that later rolls back. Consumers must still be idempotent because outbox delivery is normally at least once.
...
3. Decide whether a Specification abstraction improves your Dapper blog API. Compare it with explicit query objects for published, author, tag, and featured-post searches. Prefer direct SQL/query handlers unless the abstraction removes real duplication, and verify generated parameters and results with PostgreSQL integration tests.
</ExerciseBlock>

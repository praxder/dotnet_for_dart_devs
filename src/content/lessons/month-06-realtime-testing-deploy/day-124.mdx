...
// BETTER: use a typed response contract (this record class is still a reference type)
app.MapGet("/api/status", () =>
...
```csharp
// Reusable libraries may opt out of a caller's synchronization context.
// ASP.NET Core application code normally has no custom SynchronizationContext,
// so ConfigureAwait(false) is usually unnecessary there.
var data = await db.Products.ToListAsync().ConfigureAwait(false);

// Avoid async void — always return Task
// BAD:
public async void ProcessAsync() { /* caller cannot await completion or catch failures */ }
...
public ValueTask<string> GetCachedValueAsync(string key)
{
    if (_cache.TryGetValue(key, out var cached))
        return ValueTask.FromResult(cached);  // can avoid allocating a Task on this path

    return new ValueTask<string>(FetchFromDatabaseAsync(key));
}
```

An `async void` exception is not simply swallowed: it is posted to the current `SynchronizationContext` when one exists, or becomes an unhandled process-level exception. Reserve `async void` for framework event-handler signatures.

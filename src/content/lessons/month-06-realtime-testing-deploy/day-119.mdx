...
public class BlogMetrics
{
    ...
    public BlogMetrics(IMeterFactory meterFactory, PendingCommentCounter pendingComments)
    {
        ...
        // Observable callbacks must be fast and non-blocking; do not query PostgreSQL here.
...
// In a query handler:
public class GetPostHandler(IPostRepository posts, BlogMetrics metrics) : ...
{
...
```csharp
public class PostSearchService(IPostSearchRepository posts, BlogMetrics metrics)
{
    ...
    public async Task<List<PostSummaryDto>> SearchAsync(string query, CancellationToken ct)
    {
        ...
        try
        {
            var results = await posts.SearchAsync(query, limit: 20, ct);

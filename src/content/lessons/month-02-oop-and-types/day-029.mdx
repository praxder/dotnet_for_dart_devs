...
public class ApiService : ILoggable, IHealthCheckable
{
    ...
    {
        ((ILoggable)this).LogInfo("Starting...");
        _connected = true;
        ((ILoggable)this).LogInfo("Started");
    }
}

var svc = new ApiService();
ILoggable loggable = svc;
IHealthCheckable healthCheck = svc;
loggable.LogInfo("Hello");
Console.WriteLine(healthCheck.GetHealthReport());
```

Default interface members are dispatched through the interface. They do not become ordinary members of the implementing class, so use an interface-typed reference (or an explicit cast) when calling them.

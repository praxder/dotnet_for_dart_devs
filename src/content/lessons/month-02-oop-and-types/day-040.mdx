...
for (int i = 0; i < 1000; i++)
    builder.Add($"item{i}");
ImmutableList<string> result = builder.ToImmutable();  // efficient batch construction
// Better than repeatedly replacing an immutable list in a loop
```
...
string host = config["host"];  // optimized for repeated read-only lookup

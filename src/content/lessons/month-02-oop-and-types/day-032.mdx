...
<CodeComparison>
  ...
  <div slot="csharp">
  ```csharp
// Positional record with init-only generated properties
...
A positional `record class` declaration gives you:
- Generated `init`-only properties (unless you declare different members)
- Value-based `==` and `!=`
...
Records can have methods, additional properties, and custom constructors. Records are not inherently immutable; immutability depends on their members.
...
| Mutability | Depends on members | Depends on members | Depends on members; positional properties are init-only | Mutable by default; `readonly record struct` is immutable |

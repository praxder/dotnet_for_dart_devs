...
phase: "csharp"
dartConcept: "Dart int (platform-dependent representation), double, BigInt"
...
Dart's built-in numeric types are `int` and `double`; arbitrary-precision integers use `BigInt`. Native Dart uses signed 64-bit integers, while JavaScript targets have different representation and precision constraints. C# exposes a wider set of fixed-size numeric types plus `decimal` and `BigInteger`.
...
The `L` suffix forces a literal to be `long`. An unsuffixed integer literal is assigned the first fitting type among `int`, `uint`, `long`, and `ulong`, so a fitting conversion to `long` can still be legal.

```csharp
int tooLarge = 3000000000; // ERROR: the literal does not fit in int
long big = 3000000000;     // OK: the literal fits and converts to long
long explicitLong = 3000000000L; // OK: suffix communicates intent
```
...
Native Dart integers are signed 64-bit values, so code must not assume arbitrary precision. Use Dart `BigInt` when values can exceed that range. In C#, use `checked` where overflow must fail rather than wrap.
</ConceptCallout>
...
<CodeComparison>
  ...
  // BigInt is the explicit arbitrary-precision type
  BigInt huge = BigInt.parse('99999999999999999999999999999999');
...
<DartEquivalent>
Dart's `BigInt` and C#'s `BigInteger` are the explicit arbitrary-precision choices. Do not make correctness depend on a target runtime widening ordinary Dart `int` values.
</DartEquivalent>

...
<ConceptCallout type="different" title="Dart Loop Variables Differ">
Dart creates a fresh loop variable for each iteration of a standard `for` loop, so the direct Dart translation captures `0, 1, 2, 3, 4`. C#'s `for` loop reuses one variable. C# `foreach`, however, has per-iteration capture semantics in modern language versions.
</ConceptCallout>
...
// static lambda — explicitly prevents closure capture
...
Action<string> append = text => sb.Append(text); // adapt StringBuilder's return value to void
append("Hello");
append(" World");

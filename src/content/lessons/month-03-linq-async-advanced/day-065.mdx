...
public class OrderProcessor : IAsyncDisposable
{
    ...
    private async Task ProcessOrdersAsync(CancellationToken ct)
    {
        ...
        {
            // No more events can be produced after order processing finishes.
    ...
    {
        // Graceful shutdown: stop intake, drain orders, then drain events.
        _inboundChannel.Writer.TryComplete();
        try
        {
            await _processingTask;
            await _eventDispatchTask;
        }
        finally
        {
            // Cancellation is reserved for a separate forced-shutdown policy.
            _cts.Dispose();
        }
    }
}
...
Normal disposal drains accepted work. A production host should also define a bounded forced-shutdown path: after its grace period, cancel the processor, record unfinished work, and await both tasks so cancellation and failures are observed.

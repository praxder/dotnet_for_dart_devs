...
If you've shipped any real Dart code, `async`/`await` is already in your muscle memory. C# and Dart use very similar syntax for composing asynchronous operations. The mental model transfers well, but runtime and application-host details still matter.
...
The structure is nearly identical. The `async` keyword goes on the method, an incomplete awaited operation suspends the method without blocking its thread, and the return type represents eventual completion with `Task<T>`.

<ConceptCallout type="same">
`async`/`await` syntax is nearly identical between Dart and C#. In both languages, code runs synchronously until it reaches an incomplete awaited operation, then the method suspends so other work can proceed.
</ConceptCallout>
...
// BAD: the caller cannot await completion or observe failure through a Task
...
Task DoNothing()
{
    return Task.CompletedTask; // already completed
}
```
</CodeComparison>

<ConceptCallout type="tip">
Use `Task.CompletedTask` whenever you need to satisfy a `Task` return type but have no asynchronous work. `Task.FromResult(value)` is the corresponding tool for `Task<T>`. Treat both as completion values; do not rely on task object identity or implementation-specific caching.
</ConceptCallout>
...
This is a subtle but important concept. When a synchronization context or custom task scheduler is present, `await` normally captures it so the continuation can return there—for example, to a UI thread in a desktop app. `ConfigureAwait(false)` says the continuation does not require that captured context. It does not promise a thread switch.
...
public async Task<string> FetchDataAsync()
{
    ...
    // No request to marshal the continuation back to a captured context.
...
In both C# and Dart, an `async` method starts executing synchronously when called. It runs until it reaches an awaited operation that has not completed. In C#, that means code before the first suspension runs on the calling thread:
...
<ConceptCallout type="same">
Dart also runs an async function synchronously until its first incomplete `await`. Neither language moves CPU-bound work to a background thread merely because a method is marked `async`; use an isolate in Dart or an appropriate worker/thread-pool strategy in .NET when CPU work must leave the UI thread.
</ConceptCallout>

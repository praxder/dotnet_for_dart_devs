...
```csharp
// Dapper parameters keep values separate from SQL structure.
var name = userInput;
...
var unsafeSql = $"SELECT * FROM products WHERE name = '{name}'";
var unsafeProducts = await connection.QueryAsync<Product>(unsafeSql);

// Dynamic identifiers cannot be parameters. Map user choices to known SQL.
...
app.Use(async (context, next) =>
{
    ...
    // Disable obsolete browser XSS auditors; use output encoding and CSP instead.

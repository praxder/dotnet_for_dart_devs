...
Dapper expands list parameters and handles an empty list for supported providers, but returning early is still clearer and avoids a database round trip. Materialize one-shot enumerables before checking emptiness so you do not consume them twice.
</ConceptCallout>

...
app.MapPost("/api/auth/verify-2fa", async (
    ...
    UserManager<AppUser> userManager,
    JwtTokenService jwtService,
    ITwoFactorAttemptStore attemptStore,
    CancellationToken cancellationToken) =>
{
    ...
    if (!await attemptStore.CanAttemptAsync(userId, cancellationToken))
    ...
    if (!isValid)
    {
        await attemptStore.RecordFailureAsync(userId, cancellationToken);
        return Results.Unauthorized();
    }

    await attemptStore.ClearAsync(userId, cancellationToken);

    var tokens = await jwtService.GenerateTokensAsync(user);
    return Results.Ok(tokens);
}).RequireRateLimiting("two-factor");
```

The pre-auth token must use a separate audience or authentication scheme from normal API access, expire within minutes, contain a one-time challenge identifier, and grant no application permissions. Limit attempts by both challenge/account and network source; a valid password must not permit unlimited TOTP guessing.
...
app.MapDelete("/api/account/sessions/{id}", async (
    int id, ClaimsPrincipal user, AppIdentityDbContext db) =>
{

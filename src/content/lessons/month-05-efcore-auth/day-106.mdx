...
This course keeps that integration inside an **Identity-only context**. Blog, order, and other application data continue to use the Dapper repositories from module 8. Sharing one PostgreSQL server is fine; sharing persistence models or using the Identity context as a general application `DbContext` is not part of this architecture.

    ...
    JwtTokenService jwtService) =>
{
    ...
    if (payload is null || !payload.EmailVerified) return Results.Unauthorized();
    ...
    if (user is null)
    {
        // Never link solely because an email string matches. An existing account
        // must authenticate first and use an explicit account-link flow.
        var existingUser = await userManager.FindByEmailAsync(payload.Email);
        if (existingUser is not null)
            return Results.Conflict(new { message = "Sign in to link this provider." });
        ...
        var linkResult = await userManager.AddLoginAsync(user,
            new UserLoginInfo("Google", payload.Subject, "Google"));
        if (!linkResult.Succeeded) return Results.Problem("Failed to link login");
    }
...
3. Add GitHub OAuth login: map the username to `DisplayName` and avatar to `AvatarUrl`. If its verified email already belongs to an account, reject automatic linking and require that account to authenticate before an explicit link operation.
</ExerciseBlock>

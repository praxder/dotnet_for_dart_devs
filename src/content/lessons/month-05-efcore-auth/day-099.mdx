...
// Install separately: dotnet add package Dapper.SqlBuilder
// using Dapper;
...
`SqlBuilder` is published as the separate `Dapper.SqlBuilder` package. It uses the `Dapper` namespace, but installing the main `Dapper` package alone does not add it.

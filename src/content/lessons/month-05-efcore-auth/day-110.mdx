...
ASP.NET Core Data Protection protects app-controlled payloads such as authentication cookies and short-lived tokens. It can also protect selected database fields when its key ring is durable and shared correctly, but it is not Windows DPAPI or a general-purpose key-management service. Secrets management keeps API keys and connection strings out of source code.
...
Protected values are intentionally nondeterministic, so they cannot be queried with ordinary equality searches. For searchable sensitive fields, store a separate keyed hash for lookup and protect the original value. Back up the Data Protection key ring: losing it makes existing values unreadable.
...
// With Dapper, make the protection boundary explicit in the repository.
var protectedPhone = sensitiveData.Encrypt(user.PhoneNumber);
await connection.ExecuteAsync(
    "UPDATE users SET protected_phone = @protectedPhone WHERE id = @id",
    new { protectedPhone, id = user.Id });

var storedValue = await connection.QuerySingleAsync<string>(
    "SELECT protected_phone FROM users WHERE id = @id",
    new { id = user.Id });
var phoneNumber = sensitiveData.Decrypt(storedValue);
...
public class ApiKeyService(IApiKeyRepository repository, SecureHasher hasher)
{
    ...
    {
        var prefix = key[..12];
        var candidates = await repository.FindByPrefixAsync(prefix);
...
2. Protect a sensitive field at the Dapper repository boundary. Verify that PostgreSQL contains protected data while the application returns plaintext. Then document the recovery and re-encryption plan before changing keys or purpose strings; losing the key makes existing values unreadable.

...
```csharp
// Use DbConnection/DbTransaction when the abstraction needs async ADO.NET APIs.
// The connection and transaction belong to this operation.
public sealed class UnitOfWork(IDbConnectionFactory factory)
{
...
For most operations, you don't need a Unit of Work—open and dispose one connection in the repository method. Use a transaction wrapper only when multiple commands must commit atomically. Pass the same `DbConnection` and `DbTransaction` to every Dapper command in that operation.
</ConceptCallout>

...
app.MapGet("/api/orders/{id}", async (int id, IOrderRepository orders) =>
    await orders.GetByIdAsync(id));  // any authenticated user can see any order!
    ...
    ClaimsPrincipal user) =>
{
    ...
    var order = await orders.GetForCustomerAsync(id, userId);
...
## Runtime Guardrails and Safe Security Events

Do not try to detect SQL injection or path traversal with a blacklist. Attack strings have many encodings and valid inputs can contain suspicious-looking text. Enforce authorization and resource ownership, validate each endpoint's input model, parameterize SQL, limit request sizes, and let framework parsers handle structured input.

```csharp
// Record a bounded event when an actual security control rejects a request.
public static class SecurityEvents
{
        ...
        string outcome)
    {
        ...
        logger.LogWarning(
            "Security control {Control} rejected {Endpoint}; outcome={Outcome}; trace={TraceId}",
            control,
            endpoint,
            outcome,
...
Use normalized endpoint metadata and correlation IDs in these events. Never attach raw authorization headers, tokens, request bodies, or query-string values.
...
3. Implement a security-event test for requests to a deliberately nonexistent administrative route. Record a correlation ID, normalized route, outcome, and a non-reversible token fingerprint if correlation is necessary. Never log the raw JWT, authorization header, query-string secrets, or request body.
</ExerciseBlock>

...
Building on the Dapper blog API from module 8, add the full auth layer. Dapper remains the application data-access approach. ASP.NET Core Identity may use EF Core only as its isolated user-store adapter; keep that boundary explicit and do not introduce a second persistence model for blog data.
...
// Identity store only; application repositories continue to use Dapper.
...
builder.Services.AddRateLimiter(options =>
{
    ...
    {
        var networkKey = context.Connection.RemoteIpAddress?.ToString() ?? "unknown";
        return RateLimitPartition.GetFixedWindowLimiter(networkKey, _ => new()
        {

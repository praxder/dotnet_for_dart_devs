...
**Course baseline: .NET 10 LTS with C# 14.** Use a supported target framework and install current servicing patches. Ignore the Framework/Core/Standard history unless you are maintaining older code or publishing a compatibility library.
</ConceptCallout>
...
|---------|------|----------------|
| .NET 6 | LTS | November 12, 2024 (ended) |
...
Support dates change as releases move through servicing. Verify them on the official [.NET support policy](https://dotnet.microsoft.com/en-us/platform/support/policy) before choosing a production baseline.
...
# 5. Run it both ways
dotnet run
dotnet bin/Debug/net10.0/RuntimeExplorer.dll
```

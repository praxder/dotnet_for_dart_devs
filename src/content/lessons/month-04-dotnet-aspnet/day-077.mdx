...
// Invoke async validators explicitly from an async endpoint/action.
    ...
    CancellationToken cancellationToken) =>
{
    ...
    return Results.Created("/api/products/pending", request);
});
```

FluentValidation's ASP.NET MVC automatic-validation pipeline is synchronous. Do not combine `AddFluentValidationAutoValidation` with `MustAsync` rules: async validators cannot run correctly in that pipeline. Explicit asynchronous validation also makes I/O and cancellation visible at the endpoint boundary.

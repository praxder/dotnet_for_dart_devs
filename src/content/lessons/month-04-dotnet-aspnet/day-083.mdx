...
Minimal API endpoints that bind `IFormFile` require antiforgery services and middleware in current ASP.NET Core versions. For browser cookie authentication, issue and validate antiforgery tokens. A mobile API that exclusively uses bearer tokens is not vulnerable to cookie-based CSRF, so it may explicitly disable antiforgery on that endpoint after documenting the authentication constraint.
...
<CodeComparison>
  ...
  app.MapPost("/api/files/upload", async (IFormFile file, IWebHostEnvironment env) =>
  {
      ...
      if (!allowedTypes.TryGetValue(file.ContentType, out var extension)
          || !await FileValidator.IsValidContentTypeAsync(file, file.ContentType))
          return Results.BadRequest($"File type {file.ContentType} not allowed");

      // Generate a safe name and choose the extension from validated content.
  ...
  .DisableAntiforgery(); // Only for a bearer-token-only mobile API; never for cookie auth.
  ```
  </div>
</CodeComparison>
...
public static class FileValidator
{
        ...
        IFormFile file, string expectedContentType)
    {
        if (!MagicNumbers.TryGetValue(expectedContentType, out var magic))
            return false; // unknown types are rejected, not trusted
